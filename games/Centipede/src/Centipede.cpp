...
{
    // player move
    if (key == KEY_UP && _player[0] > 32) {
        _map[_player[0]][_player[1]] = ' ';
        _player[0]--;
        _map[_player[0]][_player[1]] = 'P';
    }
    else if (key == KEY_DOWN && _player[0] < 39) {
        _map[_player[0]][_player[1]] = ' ';
        _player[0]++;
        _map[_player[0]][_player[1]] = 'P';
    }
    else if (key == KEY_LEFT && _player[1] > 0) {
        _map[_player[0]][_player[1]] = ' ';
        _player[1]--;
        _map[_player[0]][_player[1]] = 'P';
    }
    else if (key == KEY_RIGHT && _player[1] < 39) {
        _map[_player[0]][_player[1]] = ' ';
        _player[1]++;
        _map[_player[0]][_player[1]] = 'P';
    }
    ...
    for (std::vector<std::array<int, 3UL>>::iterator i = _centipedes.begin(); i < _centipedes.end(); i++) {
        ...
        if ((*i)[2] == 1 && (*i)[1] < 39 && (_map[(*i)[0]][(*i)[1] + 1] == ' ' || _map[(*i)[0]][(*i)[1] + 1] == '|')) {
            _map[(*i)[0]][(*i)[1]] = ' ';
            (*i)[1]++;
            _map[(*i)[0]][(*i)[1]] = 'O';
        }
    ...
    if (_toAppear) {
        _toAppear--;
        _centipedes.push_back({0, 19, rand() % 2});
    }
    // lazer
    if (_lazer[0] < 0)
        _lazer = _player;
